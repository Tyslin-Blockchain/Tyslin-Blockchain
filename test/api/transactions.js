...
function sendTYS(account, amount, done) {
...
function sendTYS2voter(params, done) {
...
before(function(done) {
    setTimeout(function() {
        sendTYS(account, node.randomLISK(), done);
    }, 2000);
});

before(function(done) {
    setTimeout(function() {
        sendTYS(account2, node.randomLISK(), done);
    }, 2000);
});
...
before(function(done) {
    setTimeout(function() {
        // Send 20 LSK
        sendTYS(account2, 20 * 100000000, done);
    }, 2000);
});
...
before(function(done) {
    setTimeout(function() {
        // Send 100 LSK
        sendTYS(account2, 100 * 100000000, done);
    }, 2000);
});
    ...
    it('using entire balance should fail', function(done) {
        openAccount({ secret: account.password }, function(err, res) {
            ...
            putTransaction({
                ...
                node.expect(res.body).to.have.property('success').to.be.not.ok;
                node.expect(res.body).to.have.property('error').to.match(/Account does not have enough TYS: U[0-9]+ balance: [0-9.]+/);
                done();
            });
        });
    });
